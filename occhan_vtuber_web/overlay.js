const OVT = (() => {
  ...
  function chooseMouth(m) {
    ...
    if (jaw < 0.16 && smile > 0.28) {
      name = "mouth_smile";
    } else if (jaw > 0.55 && pucker > 0.22) {
      name = "mouth_surprise";
    } else if (jaw > 0.42) {
      name = "mouth_a";
    } else if (jaw > 0.22 && pucker > 0.25) {
      name = "mouth_o";
    } else if (jaw > 0.22) {
      name = "mouth_e";
    }
  ...
  function render() {
    ...
    stage.style.transform =
      ...
      `skewX(${state.yaw * -1.6}deg) ` +
      `scaleX(${1 - Math.abs(state.yaw) * 0.018}) ` +
      `scaleY(${1 + state.pitch * 0.006})`;

    const eyeShift = state.yaw * 1.2;

    eyeL.style.transform =
      `translateX(${eyeShift}px) ` +
      `scaleY(${Math.max(0.08, 1 - state.blinkL * 0.92)})`;

    eyeR.style.transform =
      `translateX(${eyeShift}px) ` +
      `scaleY(${Math.max(0.08, 1 - state.blinkR * 0.92)})`;

    browL.style.transform =
      `translateY(${-state.browL * 3.5}px)`;

    browR.style.transform =
      `translateY(${-state.browR * 3.5}px)`;
    ...
    if (
      ...
      try {
        ...
        if (lm) {
          ...
          const midX =
            (eyeLpt.x + eyeRpt.x) * 0.5;

          const midY =
            (eyeLpt.y + eyeRpt.y) * 0.5;
